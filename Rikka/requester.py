...
async def filter_requests(event):
    ...
    if "#request" in event.text:
      #  await asst.send_message(IN_GRP,
        ...
        #                        Button.url("🔗 Community 🔗", url="https://t.me/AcGc_01")],
        ...
        x = await asst.send_message(REQ_GO,
                                ...
                                buttons=[
                                    ...
                                    [Button.inline("⛔️ Reject", data="reqdelete"),
        ...
        if not auth:
            async for x in bot.iter_participants("@Anime_X_Clan", filter=ChannelParticipantsAdmins):
                auth.append(x.id)
...
async def delete_message(event):
    ...
    if event.sender_id in auth:
        ...
        await event.edit(f"**REJECTED**\n\n~~{xx}~~", buttons=[Button.inline("Request Rejected ⛔️", data="ndone")])
...
async def delete_message(event):
    ...
    if event.sender_id in auth:
        ...
        await tbot.send_message(-, f"**⚠️ Request Unavailable ⚠️**\n\n~~{xx}~~", buttons=btns)
    else:
        await event.answer("Who TF are you? This is for admins only..", alert=True, cache_time=0)
...
async def ans(e):
    await e.answer("This Request Is Completed... Checkout @Anime_X_Clan 💖", alert=True, cache_time=0)
...
async def ans(e):
    await e.answer("This Request is unavailable... Ask Admins in @Anime_X_Clan  for help. 💞", alert=True, cache_time=0)
